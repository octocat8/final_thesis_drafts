...
Such a modification is required for the original computation of rank (see \cref{thm:computingrankpostadjoin}) to be an equality. 
\begin{proposition}
    Given $T$ as in \cref{eqn:wangoriginaltensor}, and $\mathcal{M}$ as \cref{eqn:wangslicingfamilytensor}, replacing $\mathcal{M}$ with a family $\mathcal{W}$ such that $\mathcal{W}$ only consists of symmetric tensors and such that $T\mod\mathcal{M} = T\mod\mathcal{W}$ leads to an invalidation of \cref{eqn:familyineq} when applied to $T\mod\mathcal{W}$.
\end{proposition}
\begin{proof}
