...
\begin{definition}[Creating a Tensor from a Family of Slices]\label{defn:creatingorderd}
    ...
    %     \bar{\mathcal{M}}(k_1, \ldots k_{i-1}, j, k_{i+1} \ldots , k_d):= M_j(k_1, \ldots k_{i-1}, k_{i+1} \ldots k_d).
    % \end{equation*}
    Let $d > 0$ and $\mathcal{M} = \{M_1, \ldots, M_N\} \in V_{\hat{i}}$ be a finite collection of order-$(d-1)$ tensors. Then each $M_j$ can be written in the following manner $$M_j := \sum_{k=1}^{r_j} m^k_{j, 1} \otimes \cdots \otimes m^k_{j, i-1} \otimes m^k_{j, i+1} \otimes \cdots \otimes m^k_{j, d}.$$ Then the operation $$\bar{\cdot}^i: \times_{i=1}^N V_{\hat{i}} \to V_1 \otimes \cdots \otimes V_{i-1} \otimes \field^N \otimes V_{i+1} \otimes \cdots \otimes V_d,$$ defined by taking (as a tuple) $$\mathcal{M} \mapsto \sum_{j=1}^{N} \sum_{k=1}^{r_j}  m^k_{j, 1} \otimes \cdots \otimes m^k_{j, i-1}\otimes e_j \otimes m^k_{j, i+1} \otimes \cdots \otimes m^k_{j, d},$$ is a well-defined, bijective operation.
\end{definition}
...
\begin{definition}[Adjoining Slices to a Tensor along a specific Mode]\label{defn:adjoiningslicesinglemode}
    ...
    % \hrule
    Let $T \in V_1 \otimes \cdots \otimes V_d$ be a tensor, with format $(n_1, \ldots, n_d)$. Let $\mathcal{M} = \{M_1, \ldots, M_N\}$ be a collection of order-$(d-1)$ tensors lying in $V_{\hat{i}}$. Then define the following set $$\tau := \{S^i_j(T)\}_{j=1}^{n_i} \sqcup \mathcal{M}.$$ Without a loss of generality, this set can be fit into a tuple of length $n_i + N$, with the first $n_i$ entries describing the $i$-slices of $T$ in order, and the last $N$ entries the elements of $\mathcal{M}$ in order.
...
\begin{remark}
    Since the above definition relies on a coordinate dependent operation, it is similarly not possible to describe such an operation in a coordinate independent manner.
\end{remark}
...
\begin{definition}[Padding an order-$(d-1)$ tensor without increasing order]\label{defn:lowerorderpadding}
    ...
    % \end{cases}$$
    % \hrule
    Let $d > 0$ be given, and $T \in V_1 \otimes \cdots \otimes V_d$ be the candidate tensor of format $(n_1, \ldots, n_d)$. It is according to this format that the given $A \in W_{\hat{i}}$ is to be padded. Now $$A = \sum_{k=1}^{r} w_1^k \otimes \cdots \otimes w_{i-1}^k \otimes w_{i+1}^k \otimes \cdots \otimes w_d^k, w_j^k \in W_j \; \forall j = 1, \ldots, d.$$ Here $W_j \leq V_j$ for all $1 \leq i \leq d$, i.e. they are all subspaces of the $V_j$. $A$ therefore has a format of $(m_1, \ldots, m_{i-1}, m_{i+1}, \ldots, m_d)$ where $m_j \leq n_j.$ There exists a canonical injection $I_j: W_j \hookrightarrow V_j$ for all $j = 1, \ldots, d$, that appends $0$'s to each vector under consideration. Define the padding operation as $$pad : W_{\hat{i}} \times V \to V_{\hat{i}}$$ taking $$(A, T) \mapsto \sum_{k=1}^{r} I_1(w_1^k) \otimes \cdots \otimes I_{i-1}(w_{i-1}^k) \otimes I_{i+1}(w_{i+1}^k) \otimes \cdots \otimes I_d(w_d^k).$$ 
\end{definition}
\begin{example}
...
\begin{flushleft}
    ...
    \textbf{Input:} $d > 0$. A tensor $T \in W_1 \otimes \cdots \otimes W_d$ with format $(m_1, \ldots, m_d)$. Let $\mathcal{M}_i \subset V_{\hat{i}}$ be finite collections of order-$(d-1)$ tensors, each containing $N_i$ slices.
\end{flushleft}
\begin{flushleft}
    \textbf{Output:} $Adj(T, \mathcal{M}_1, \ldots, \mathcal{M}_d)$
    \begin{enumerate}
        \item Let $i = 1$. Let $\tau^0 = T$. 
        \item Define $\tau^i: =Adj^i(\{S_i^j(\tau^{i-1})\}_{j=1}^{n_i}\sqcup \mathcal{M}_i)$. Pass to Step (3)
        \item Redefine $\mathcal{M}_j := \{ pad(M_j^k, \tau_i)\}_{k=1}^{N_j}$ for each $j > i$. Pass to Step (4)
        \item If $i \leq d$, set $i = i + 1$ and pass to step (2)
        \item Return $\tau_d$
    \end{enumerate}
\end{flushleft}
\hrule
\vspace{12pt}
The above algorithm defines the operation $Adj: W_1 \otimes \cdots \otimes W_d \times \left(\times_{i=1}^d \bar{W_j}(N_j)\right) \to V_1 \otimes \cdots \otimes V_d$.
...
\begin{remark}
    The algorithm written above can be modified to work for any ordering of modes. Let $\sigma \in S_d$ be a permutation of $\{1, \ldots, d\}$, describing the new order of adjoining. The following steps must then be changed to work with this modified ordering: \begin{itemize}
        ...
        \item Step (3) changes to "Redefine $\mathcal{M}_j := \{ pad(M_j^k, \tau_i)\}_{k=1}^{N_j}$ for each $j \notin \{\sigma(k)\}_{k=1}^i$. Pass to Step (4)"
    \end{itemize}
...
The \textbf{symmetric adjoin} can be denoted $SAdj(T, \mathcal{M})$ where $\mathcal{M}$ is adjoined along every mode, following the same process as given in \Cref{subsection:adjoiningprocedure}. Here $SAdj(T, \mathcal{M}) := Adj(T, \mathcal{M}, \ldots, \mathcal{M})$. The collection $\mathcal{M}$ consists entirely of symmetric order-$(d-1)$ tensors as well.
...
\begin{definition}[Padding an order-$(d-1)$ tensor into higher order]\label{defn:paddinghigherorder}
    ...
    % \end{cases}$$
    % \hrule
    Let $d > 0$, and $T \in V_1 \otimes \cdots \otimes V_d$, having a format $(n_1, \ldots, n_d)$. Let $A \in V_{\hat{i}}$ be an order-$(d-1)$ tensor, of the format $(n_1, \ldots, n_{i-1}, n_{i+1}, \ldots n_d)$, be of the following form: $$A := \sum_{k=1}^{r} v_1^k \otimes \cdots \otimes v_{i-1}^k \otimes v_{i+1}^k \otimes \cdots \otimes v_d^k, v_j^k \in V_j \;\forall\; j = 1, \ldots, i-1, i+1, \ldots, d.$$
...
As seen in the previous two subsections, rank information of a tensor does change when adjoining slices, and the following theorem is becomes important in Wang and Seigal's 2023 paper \cite{seigal_ranks_2020}. In their paper, it is stated as a corollary to the real substitution method as defined in Lemma B.1 (also known as the layer reduction method) from \cite{alexeev_tensor_2011}.
...
\begin{lemma}[The substitution method \cites{landsberg_abelian_2017,seigal_ranks_2020,shitov_comons_2020,wang_lower_2023}]\label{lemma:substitutionmethod}
    Let $T \in V_1 \otimes \cdots \otimes V_d$ be an order-$d$ tensor. $T$ can be written in terms of basis elements of $V_1$ in the following manner $$T = \sum_{k=1}^{n_1} e_k \otimes M_k, M_k \in V_{\hat{1}},$$ where $\{e_k\}_{k=1}^{n_1}$ forms a basis for $V_1$. Suppose $M_1 \neq 0$. Then, there exist constants $\lambda_2, \ldots, \lambda_{n_1}$ such that the tensor $$\tau := \sum_{k=2}^{n_1} e_k \otimes (M_k - \lambda_kM_1)$$ exists having $\rank(\tau) \leq \rank(T) - 1$. Equality holds when $M_1$ is an elementary tensor.
\end{lemma}
\begin{proof}
    Suppose $T$ has rank $r$. Then $T := \sum_{i=1}^{r} T_i$. Each of these elementary $T_i$ can be further expressed in terms of the basis vectors for $V_1$ in the following manner $T_i = \sum_{k=1}^{n_1} a_{i,k}e_k \otimes L_k$, where each $L_k$ is an elementary tensor in $V_{\hat{1}}$. Thus each $1$-slice of $T$, given by $M_k$ can be written as $M_k = \sum_{i=1}^{r} a_{i,k}L_k$. Thus in particular, since $M_1 \neq 0$, there is at least one $a_{i, 1}$ is non-zero. Without a loss of generality, assume $a_{1,1} \neq 0$. Then setting each of the $\lambda_k = a_{1, k}$ the tensor $$\tau := \sum_{k=2}^{n_1} e_k \otimes (M_k - \lambda_kM_1)$$ has all its slices $M_k - \lambda_kM_1$ expressible as a linear combination of the elementary $L_2, \ldots, L_k$. Therefore, $\rank(\tau) \leq r - 1$. Now if $M_1$ was elementary, subtracting multiples of it from $T$ only changes the rank by at most $1$. 
\end{proof}
Such coefficients can be found along any mode, for any finite collection of slices (not necessarily the ones that make up the original tensor, see Corollary B.2 in \cite{alexeev_tensor_2011}). Thus one has the following inequality $$\min \rank(T\mod(\mathcal{M}_1, \ldots, \mathcal{M}_d)) \leq \rank(T) - \sum_{i=1}^{d} \dim \langle \mathcal{M}_i \rangle.$$
...
\end{theorem}
\begin{proof}
    Suppose $T$ is an order-$d$ tensor, having $\rank(T) = r$, and each $\mathcal{M}_i := \{M_{i, k}\}_{k=1}^{N_i}$. Then the order-$d$ tensor created by adjoining all of these slices along the appropriate modes is given as $$\tau := Adj(T, \mathcal{M}_1, \ldots, \mathcal{M}_d).$$ The goal of this proof is to show that the adjoining of slices along all modes can be characterised in terms of the way these $d$ distinct slice families interact with the original tensor $T$.
    Consider the following relationship seen as a consequence of the LEMMA FROM CHAPTER 2$\rank(\tau) = \max_{1 \leq i \leq d} \dim \mathcal{L}_i(\tau)$. Here $$\mathcal{L}_i(\tau):= span \left(\{ S^i_j(\tau)\}_{j=1}^{n_i} \sqcup \{ pad^i(M_{i,k}, \tau) \}_{k=1}^{N_i}\right),$$ where first the $i$-slices containing the information from $T$ are selected from $\tau$, and then the appropriately padded slices containing new information. However, the non-trivial parts of the slices $pad^i(M_{i,k}, \tau)$ only interact with the $S^i_j(T)$ parts of the slices $S^i_j(\tau)$, and so it is seen that $$\dim \mathcal{L}_i(\tau) = \dim span(\{S^i_j(T)\}_{j=1}^{n_i} \sqcup \{M_{i,k}\}_{k=1}^{N_i}).$$ By Lemma 4.1, this can be seen as the upper bound of $$\min \rank(T\mod(\emptyset, \ldots, \emptyset,\mathcal{M}_i,\emptyset, \ldots, \emptyset)) + \dim \langle \mathcal{M}_i \rangle.$$ Therefore the following inequality exists where $$\rank(\tau) \geq \max_{1 \leq i \leq d} \min \rank(T\mod(\emptyset, \ldots, \emptyset,\mathcal{M}_i,\emptyset, \ldots, \emptyset)) + \dim \langle \mathcal{M}_i \rangle.$$ This inequality describes the relationship between the adjoined construction, and the original when altered only along one mode. Such an inequality persists over alterations over any collection of modes. This is because the original characterisation of $\rank(\tau)$ in terms of the slice spaces could also have been done in terms of the ranks of all possible order-$2$ tensors contained within, and even in terms of all possible order-$d$ tensors. Thus one can replace the $\max_{1 \leq i \leq d} \min \rank(T\mod(\emptyset, \ldots, \emptyset,\mathcal{M}_i,\emptyset, \ldots, \emptyset)) + \dim \langle \mathcal{M}_i \rangle$ term with $$\min \rank(T\mod(\mathcal{M}_1, \ldots, \mathcal{M}_d)) + \sum_{i=1}^{d}\dim \langle \mathcal{M}_i \rangle.$$

    The equality portion of this relationship when all the $\mathcal{M}_i$ are made up of elementary tensors holds, again because Lemma 4.1 shows equality when elementary tensors are used. 
\end{proof}
...
The solution proposed originally by Shitov \cite{shitov_comons_2020}, is a modification of $\mathcal{M}$ to a family of \textit{elementary} order-$(d-1)$ tensors that span the same linear space. Unfortunately, in the format $(\reals^2)^{\otimes 4}$, doing so also violates the vital inequality required for rank and symmetric rank of the family $T \mod \mathcal{M}$ (see \Cref{eqn:familyineq}).

Such a modification is required for the original computation of rank to be an equality (see \Cref{thm:computingrankpostadjoin}), because equality only holds when the family consists of elementary tensors. 
...
The following proposition is adapted from Proposition 4.2 in \cite{wang_lower_2023}. It shows that for orders $d = 2n, n > 2$, modifying the initial family $\mathcal{M}$ to a finite family of elementary tensors $\mathcal{W}$ is possible while maintaining the inequality \cref{eqn:familyineq}. This argument holds specifically for the order $6$, though it does not appear to be addressed in their paper. 
...
\begin{proof}
    ...
    \end{equation} where $t_{ij} := \frac{\lambda_i\mu_j}{\mu_i\lambda_j}$, then equation 4.10 is false. However, the range of the function, seen in equation 4.11, is the entirety of $\reals$ whenever dealing with a value of $d > 4$ since the function generated in terms of $\{t_{ij}, t_{ij}^{-1}\}$ is no longer univariate.
...
The later paper \cite{wang_lower_2023} constructs this family for their candidate counterexample of order $6$. The elements of this familiy are described below for the sake of completeness. In personal correspondence, the authors indicated that this falls in step with the construction in the preprint \cite{shitov_comons_2020}:

They begin with vectors lying in $\reals^{14}$ of the form $a_i$, where $i \in [n=7]$ and each vector is such that $a_i(2i) = a_i(2i - 1) = 1$ and all the other entries are $0$. Then the new vectors constructed lie in $\reals^{28}$ and are one of the following forms (where the $|$ indicates the distinction between the first $14$ entries and the last $14$ entries.) \begin{enumerate}
