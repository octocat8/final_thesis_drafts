...
\begin{remark}
    ...
    % For a tensor $T \in V_1 \otimes \cdots \otimes V_d $, $T^{\emptyset}$ denotes a vector lying in $V_1 \oplus \cdots \oplus V_d$ with $T(k_1, \ldots, k_d) = T^{\emptyset}(\sum_{i=1}^{d}k_i(n_i-1)) $
\end{remark}
...
\end{remark} 
Another description of cloning is by first transforming the original by tensor product.	Denote $\mathbf{1}_n$ as the vector of 1's lying in $\mathbb{F}^n$\ Another way to look at $C_n(T)$ is as follows: 
\begin{itemize}
...
\end{itemize}
What are the properties of the cloning operation? 
