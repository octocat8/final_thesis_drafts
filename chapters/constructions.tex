...
Such a modification is required for the original computation of rank (see \cref{thm:computingrankpostadjoin}) to be an equality. The following proposition is adapted from \cite{wang_lower_2023}, to show that modifying $\mathcal{M}$ in such a direct way violates \cref{eqn:familyineq}. It is enough to show that an elementary symmetric tensor lies in the linear space generated with the modified family.
\begin{proposition}
    Consider $T$ as in \cref{eqn:wangoriginaltensor}, and $\mathcal{M}$ as \cref{eqn:wangslicingfamilypoly}. Let $\mathcal{W} \subset (\reals^2)^{\otimes d-1}$, a finite set of elementary symmetric tensors such that $\mathcal{W}$ spans $\mathcal{M}$. Then $0 \in T\mod\mathcal{W}$ CHANGE THIS
\end{proposition}
\begin{proof}
...
It is past this point that both Shitov, and Wang and Seigal, go forward with the construction of a family $\mathcal{W}$ to replace $C_k(\mathcal{M})$ that can be described algorithmically, but the justification for why such choices are made remains opaque. 

The later paper \cite{wang_lower_2023} constructs this family for their candidate counterexample of order $6$ as follows:

They begin with vectors lying in $\reals^{2n}$ of the form $a_i$, where $i \in \{1, \ldots, n\}$ and each vector is such that $a_i(2i) = a_i(2i - 1) = 1$ and all the other entries are $0$. Then the new vectors constructed lie in $\reals^{4n}$ and are one of the following forms (where the $|$ denotes the distinction between the first $2n$ entries and the last $2n$ entries.) \begin{enumerate}
