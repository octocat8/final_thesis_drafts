...
\begin{definition}[Creating a Tensor from a Collection of Slices]\label{defn:creatingorderd}
    ...
    Then the operation $$\bar{\cdot}^i: \times_{i=1}^N V_{\hat{i}} \to V_1 \otimes \cdots \otimes V_{i-1} \otimes \field^N \otimes V_{i+1} \otimes \cdots \otimes V_d,$$ defined by taking (as a tuple) $$(M_1, \ldots, M_N) = \mathcal{M} \mapsto \sum_{j=1}^{N} \sum_{k=1}^{r_j}  m^k_{j, 1} \otimes \cdots \otimes m^k_{j, i-1}\otimes e_j \otimes m^k_{j, i+1} \otimes \cdots \otimes m^k_{j, d},$$ is a well-defined, bijective operation.
\end{definition}
...
\begin{definition}[Adjoining Slices to a Tensor along a specific Mode]\label{defn:adjoiningslicesinglemode}
    ...
    % \hrule
    Let $T \in V_1 \otimes \cdots \otimes V_d$ be a tensor, with format $(n_1, \ldots, n_d)$. Let $\mathcal{M} = \{M_1, \ldots, M_N\}$ be a collection of order-$(d-1)$ tensors lying in $V_{\hat{i}}$. Then define the following set $$\tau := \{S^i_j(T)\}_{j=1}^{n_i} \sqcup \mathcal{M}.$$ This set can be fit into a tuple of length $n_i + N$, with the first $n_i$ entries describing the $i$-slices of $T$ in order, and the last $N$ entries the elements of $\mathcal{M}$ in order.
...
\begin{proof}
   \begin{eqnarray*}
        ...
        &=& \left\{S + \sum_{a=1}^{n_j}\sum_{b=1}^{N_j}\lambda_{a,b} Pad^j_a(S^j_b(\bar{\mathcal{M}_j}),T) | \lambda_{a,b} \in \field, S \in Mod^i(T,\bar{\mathcal{M}_i}) \right\}\\
        &=& \left\{T + \sum_{c=1}^{n_i}\sum_{d=1}^{N_i}\lambda_{c,d} Pad^i_c(S^i_d(\bar{\mathcal{M}_i}),T) + \sum_{b=1}^{n_j}\sum_{a=1}^{N_j}\lambda_{a,b} Pad^j_a(S^j_b(\bar{\mathcal{M}_j}),T) | \lambda_{a,b}, \lambda_{c,d}\in \field \right\}\\
        &=& \left\{S + \sum_{c=1}^{n_i}\sum_{d=1}^{N_i}\lambda_{c,d} Pad^i_c(S^i_d(\bar{\mathcal{M}_i}),T) | \lambda_{c,d} \in \field, S \in Mod^j(T,\bar{\mathcal{M}_j}) \right\}\\
...
\begin{observation}
    Here, the family $T\mod(\mathcal{M}_1, \ldots, \mathcal{M}_d) \subseteq V_1 \otimes \cdots \otimes V_d$, i.e. all its elements remain of the same order as the original tensor $T$. \Cref{figure:familymoduloslices} illustrates how one one such element from such a family can be generated.
\end{observation}
\begin{remark}
...
As seen in the previous two subsections, rank information of a tensor does change when adjoining slices, and the following theorem is becomes important in Wang and Seigal's 2023 paper \cite{seigal_ranks_2020}. In their paper, it is stated as a corollary to the real substitution method as defined in Lemma B.1 from \cite{alexeev_tensor_2011} (also known as the layer reduction method).

Theorem 4.4 in \cite{seigal_ranks_2020} and Proposition 3.1 in \cite{landsberg_abelian_2017} follow \cite{alexeev_tensor_2011} in providing a proof for showing how the rank of a tensor changes when slices are modified along only one mode.
...
\begin{lemma}\label{lemma:rankinternalslices}
    For any tensor $T \in V_1 \otimes \cdots \otimes V_d$, the following statement always holds $$\min \rank(T\mod(\{S^1_{j_1}(T)\}_{j_1 = 1}^{n_1}, \ldots, \{S^d_{j_d}(T)\}_{j_d = 1}^{n_d})) = 0$$
\end{lemma}
\begin{proof}
    By \Cref{lemma:substitutionmethod}, modifying a tensor $T$ by a slice it contains, $S^i_j(T)$, lowers the rank of the tensor by at least 1. Modifying $T$ by all such slices $\{S^i_j(T)\}_{j=1}^{n_i}$ lowers the rank by $n_i - \dim \langle S_j^i \rangle_{j=1}^{n_i}$. For at least one mode $i$, the number of linearly independent slices is exactly the rank of the tensor (\Cref{lemma:rankrelationshipwithonemode}). This leads to the $0$-tensor lying in the family $T\mod(\{S^1_{j_1}(T)\}_{j_1 = 1}^{n_1}, \ldots, \{S^d_{j_d}(T)\}_{j_d = 1}^{n_d})$.
\end{proof}
...
\begin{proof}
    ...
    Consider the following relationship seen as a consequence of \Cref{lemma:rankrelationshipwithonemode}, $\rank(\tau) = \max_{1 \leq i \leq d} \dim \mathcal{L}_i(\tau)$. Here $$\mathcal{L}_i(\tau):= span \left(\{ S^i_j(\tau)\}_{j=1}^{n_i} \sqcup \{ pad^i(M_{i,k}, \tau) \}_{k=1}^{N_i}\right),$$ where first the $i$-slices containing the information from $T$ are selected from $\tau$, and then the appropriately padded slices containing new information. However, the non-trivial parts of the slices $pad^i(M_{i,k}, \tau)$ only interact with the $S^i_j(T)$ parts of the slices $S^i_j(\tau)$, and so it is seen that $$\dim \mathcal{L}_i(\tau) = \dim span(\{S^i_j(T)\}_{j=1}^{n_i} \sqcup \{M_{i,k}\}_{k=1}^{N_i}).$$ By Lemma 4.1, this can be seen as the upper bound of $$\min \rank(T\mod(\emptyset, \ldots, \emptyset,\mathcal{M}_i,\emptyset, \ldots, \emptyset)) + \dim \langle \mathcal{M}_i \rangle.$$ Therefore the following inequality exists where $$\rank(\tau) \geq \max_{1 \leq i \leq d} \min \rank(T\mod(\emptyset, \ldots, \emptyset,\mathcal{M}_i,\emptyset, \ldots, \emptyset)) + \dim \langle \mathcal{M}_i \rangle.$$ This inequality describes the relationship between the adjoined construction, and the original when altered only along one mode. Such an inequality persists over alterations over any collection of modes. This is because the characterisation of $\rank(\tau)$ in terms of the slice spaces, $\mathcal{L}_i(\tau)$, could also have been done in terms of the ranks of all possible order-$2$ tensors contained within, and even in terms of all possible order-$d$ tensors. Thus one can replace the $\max_{1 \leq i \leq d} \min \rank(T\mod(\emptyset, \ldots, \emptyset,\mathcal{M}_i,\emptyset, \ldots, \emptyset)) + \dim \langle \mathcal{M}_i \rangle$ term with $$\min \rank(T\mod(\mathcal{M}_1, \ldots, \mathcal{M}_d)) + \sum_{i=1}^{d}\dim \langle \mathcal{M}_i \rangle.$$
...
Their particular tensor under consideration is represented by the homogenous polynomial $x^d - ky^d$ where $d > 0$ is a fixed, even integer, and $k > 0$ (also fixed). Such a homogenous polynomial corresponds to the following tensor \begin{equation}\label{eqn:wangoriginaltensor}
...
\begin{proof}
    ...
    This set of equalities results in the statement \begin{equation}
        1 = \frac{\lambda_1}{\mu_1}\frac{\mu_1}{\lambda_1} = \left(\sum_{i=2}^d \frac{\lambda_i}{\mu_i}\right)\left(\sum_{i=2}^d \frac{\mu_i}{\lambda_i}\right) = (d-2) + \sum_{i=2}^d \sum_{j\neq i} \frac{\lambda_i\mu_j}{\mu_i\lambda_j}.
    \end{equation} 
    
    If one can show that \begin{equation}
        \sum_{i=2}^d \sum_{j < i} t_{ij} + t_{ij}^{-1} + d - 3\neq 0, 
    \end{equation} where $t_{ij} := \frac{\lambda_i\mu_j}{\mu_i\lambda_j}$, then Equation 4.16 is false. However, the range of the function, seen in Equation 4.17, is the entirety of $\reals$ whenever dealing with a value of $d > 4$ since the function generated in terms of $\{t_{ij}, t_{ij}^{-1}\}$ is no longer univariate.
...
The later paper \cite{wang_lower_2023} constructs this family $\mathcal{W}$ for their candidate counterexample of order $6$. The elements of this familiy are described below for the sake of completeness. In personal correspondence, the authors indicated that this falls in step with the construction in the preprint \cite{shitov_comons_2020}, and they were able to verify the difference in rank and symmetric rank via MATLAB.

They begin with vectors lying in $\reals^{14}$ of the form $a_i$, where $i \in [n=7]$. Each vector is such that $a_i(2i) = a_i(2i - 1) = 1$, with all other entries are $0$. Then the new vectors constructed lie in $\reals^{28}$ and fall into one of the following 16 distinct cases. Note: the $|$ indicates the distinction between the first $14$ entries and the last $14$ entries. \begin{enumerate}
