...
\begin{document}
The title of this thesis centers around an open question about tensors. Tensors are mathematical tools, used to describe multiple linear relationships that might exist between mathematical objects. The ability of tensors to describe such interrelated information spurs the developement of \textit{multi}-linear algebra. The focus of this thesis is threefold: how are tensors constructed, where do they live when seen as mathematical objects, and how they can be broken down? 

The current state of Comon's conjecture (an open problem for the past 20 years) is hard to describe succinctly. While mathematicians have steadily increased the number of cases for which the conjecture remains valid, an answer to the question in full generality for $\reals$- or $\complex$-tensors remains unknown. The hope of this thesis is that at least some of the work done in recent years can be highlighted as useful, and paves the way towards either a definitive counterexample, or a proof of the conjecture.
\begin{figure}
    \centering
...
\begin{conjecture}[Comon, (Oeding 2008 \cite{oeding_report_2008})]
...
\begin{description}
    \item[Chapter 2] covers some preliminary terminology and definitions required to understand the history of attempts to prove Comon's conjecture. Personal contributions in this chapter include the proof of a theorem showing the isomorphism between symmetric tensors of fixed order and homogenous polynomials of fixed degree. [ADD ANY OTHER PERSONAL CONTRIBUTIONS HERE]
    \item[Chapter 3] goes over the literature that currently exists since the statement of the problem in 2008. The papers discussed highlight the valid attempts at proving Comon's conjecture over $\reals$ and $\complex$. Personal contributions to this chapter are the compilation of all these results in a singular location, highlighting the state of the art currently. [PROOFS INCLUDED IN THIS CHAPTER ARE NOT PERSONAL, OTHERWISE SPECIFY HERE]
    \item[Chapter 4] focuses on understanding Shitov's methodology and approach to constructing a counterexample to Comon's conjecture. The culmination of this chapter is an examination of the paper from Wang and Seigal \cite{wang_lower_2023}, a published real-valued counterexample to the conjecture. Personal contributions to this chapter include explicit proofs for the various operations discussed, most of which do not exist in the existing literature. [PROVIDE EXAMPLES] 
    \item[Chapter 5] highlights the gaps present in the literature so far [WHAT ARE THESE], illustrates problems with the current approach to constructing counterexamples, and seeks to highlight potential future avenues to solving this problem once and for all. [COPY PASTE THIS TO THE FINAL CHAPTER TO KNOW WHAT TO FOCUS ON WHEN FINISHING WRITING THIS THESIS]  
\end{description}
