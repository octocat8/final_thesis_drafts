...
Thus, for any given tensor $T$, there can exist multiple decompositions, not just depending on the choice of basis vectors for each $V_i$. Two questions naturally follow this definition of tensor decomposition: \begin{description}
    ...
    \item[Q2. ] If such a shortest decomposition exists, is it unique (upto the choice of a different set of basis vectors)?
\end{description}
...
\begin{lemma}[\cite{comon_symmetric_2008}]\label{lemma:symmetricdecompositionexistence}
    Let $T$ be a symmetric, order-$d$ tensor lying in $(\field^n)^{\otimes d}$. Then there exist vectors $v_1, \ldots, v_s \in \field^n$ such that 
    \begin{equation}\label{eqn:symmetrictensordecomposition}
        T = \sum_{i=1}^{s} v_i^{\otimes d}.
    \end{equation}
\end{lemma}
\begin{proof}
    Suppose the lemma is false. Then there must exist a symmetric, order-$d$ tensor $T$ that cannot be expressed in the form $\sum_{i=1}^{s} v_i^{\otimes d}$ for any $s > 0$. Then $T$ can only be expressed as an infinite sum of symmetric tensors. Since the space $V_1 \otimes \cdots \otimes V_d$ is finite dimensional, such a sum can be expressed in a finite manner, leading to a contradiction of the initial assumption that the lemma was false.
...
Tensors are not just algebraic objects, but also as points lying in a space equipped with a topology and geometry. Just as vectors and matrices can be seen as points lying in a vector space, higher-order tensors can be viewed in a similar manner. 
...
The map from $V_i \setminus \{0\} \to \proj V_i$ is defined as taking $v_i \mapsto [v_i(1): \ldots :v_i(n)]$ where the image is expressed in homogenous coordinates. Note here that when moving to the $\proj V_i$, even though the number of coordinates used to describe $v_i$ is $n_i$, the space $\proj V_i$ has dimension $(n_i - 1)$.
...
\end{definition}
\begin{remark}
    All points on the Segre variety correspond to the non-zero elementary tensors in $V_1 \otimes \cdots \otimes V_d$, upto scalar multiplication. 
\end{remark}
...
\end{remark}
\begin{remark}
    As with tensors of rank $r$ being visualised as lying in $\sigma_r(\Sigma(\proj V_1 \times \cdots \times \proj V_d))$, a similar operation can be applied to show that symmetric tensors of symmetric rank $r$ to see that they lie in $\sigma_r(\nu_d(\proj V))$.
\end{remark}
...
\begin{lemma}\label{lemma:rankrelationshipwithonemode}
    Given a tensor $T \in V_1 \otimes \cdots\otimes V_d$, such that $\rank(T) = r$, then there is at least one mode $1 \leq i \leq d$ having $r$ linearly independent slices $T^i_j$ where $1 \leq j \leq n_i$.
\end{lemma}
\begin{proof}
...
\begin{definition}[$i$-th slice space of $T$]\label{defn:slicespace}
    ...
    \begin{equation}\label{eqn:slicespacedefn}
        \mathcal{L}_i := \langle T^i_j \rangle_{j=1}^{n_1} \subseteq V_{\hat{i}},
    \end{equation}
