...
In fact, for a general order $d$ tensor $T \in V_1 \otimes \cdots \otimes V_d$, one can form a similar $d$-way array to represent it, using the fact that every tensor can be written into a sum elementary tensors. (see \Cref{defn:decomposition}) [WHY? OR RATHER EXPLAIN HOW?]
\begin{definition}[Modes of a Tensor]
    For a tensor $T \in V_1 \otimes \cdots \otimes V_d$, the \textbf{modes} of $T$ refer to the $d$ distinct indices that make up the tuples indicating elements in the $d$-way array that represents $T$. [IS THERE A WAY TO MAKE THIS DEFINITION INDEPENDENT OF COORDINATES?]
\end{definition}
\begin{remark}
    The idea of modes is equivalent to specifying an orientation or direction of a tensor when represented as a $d$-way array.
\end{remark}
\begin{example}
...
A tensor $T:= v_1 \otimes \cdots \otimes v_d \in V_1 \otimes \cdots \otimes V_d$ can be viewed in the following different ways: \begin{enumerate}\label{list:viewingtensors}
    ...
    \item For every choice of $i\in \{1, \ldots, d\}$, $T$ can be viewed as a linear mapping from $V_i^* \to V_{\hat{i}} := V_1 \otimes \cdots \otimes V_{i-1} \otimes V_{i+1} \otimes \cdots \otimes V_d$. Viewing/using $T$ this way takes a vector (an order-$1$ tensor) to an order-$(d-1)$ tensor. \begin{equation}\label{eqn:tensormappingdroporder}
    ...
    \item Finally, as the dual of the defined linear map (see \Cref{eqn:tensormappingdroporder}), now going from $V_{\hat{i}}^* \to V_i$. This point of view of $T$ transfroms an order-$(d-1)$ tensor $T$ to an a vector, collinear to the chosen $v_i$. $$(f_j)_{j \neq i} \mapsto \left(\prod_{j \neq i} f_j(v_j)\right)v_i.$$
\end{enumerate}
...
Using these elementary tensors as building blocks provides a way to describe the construction of any general order-$d$ tensors. (In the most naive way, this can be done by considering all elementary tensors of the form $e_{1, j_1} \otimes \cdots \otimes e_{d, j_d}$)
...
\end{remark}
Therefore, for any given tensor $T$, there can exist multiple decompositions, not just depending on the choice of basis vectors for each $V_i$. Two questions naturally follow this definition \begin{description}
...
\begin{definition}[Permutation of a Tensor]\label{defn:tensorpermutation}
    ...
    Let $T \in V: = V_1 \otimes \cdots \otimes V_d$ be an order-$d$ tensor. Then $\sigma \in S_d$ acts on the space $V$ in the following manner
    $$\sigma: V_1 \otimes \cdots \otimes V_d \to V_{\sigma_1} \otimes \cdots \otimes V_{\sigma_d},$$inducing a corresponding tranformation from $T \mapsto \sigma \cdot T$. [HERE THE ACTION ON AN ELEMENTARY TENSOR CAN BE SPECIFIED]
\end{definition}
...
\end{definition}
\begin{remark}
    As a consequence of this definition, the following equivalent equality condition also holds for symmetric tensors $$T = \frac{1}{d!} \sum_{\sigma \in S_d} \sigma \cdot T.$$ This gives rise to what can be called the \textbf{symmetrisation operator} $S: V \to V$ that takes $T \mapsto S(T) := \frac{1}{d!} \sum_{\sigma \in S_d} \sigma \cdot T$. This resulting tensor $S(T)$ is always symmetric.
\end{remark}
...
\begin{proof}
    ...
    [INSERT INJECTIVITY ARGUMENT HERE]
