...
\begin{definition}[Modes of a Tensor]
    ...
    Given an order-$d$ tensor, $T \in V_1 \otimes \cdots \otimes V_d$, the modes of $T$ refer to specifying the $d$ distinct ways that $T$ as a tensor can be oriented. Thus a tensor of order $n$ has exactly $n$ modes.
...
\end{lemma}
\begin{proof}
    Suppose the lemma is false. Then there must exist a symmetric, order-$d$ tensor $T$ that cannot be expressed in the form $\sum_{i=1}^{s} v_i^{\otimes d}$ for any $s > 0$. Then either $T$ is asymmetric (a violation of the initial assumption that the lemma was false, leading to a contradiction), or that $T$ can only be expressed as an infinite sum of symmetric tensors. But since the space $V_1 \otimes \cdots \otimes V_d$ is finite dimensional, such a sum can be expressed in a finite manner, again leading to a contradiction to our initial assumption.
    Therefore, a symmetric decomposition must always exist for $T$, a symmetric order-$d$ tensor.
\end{proof}
\begin{remark}
    The proof given in \cite{comon_symmetric_2008} involves showing that the space generated by all $k$-th tensor powers of linear forms in the symmetric tensors cannot be contained in any non-zero non-trivial hyperplane. Their proof also involves defining a bilinear form on the space of the symmetric tensors.
\end{remark}
\begin{example}
...
\end{theorem}
\begin{proof}
    Let $1 \leq i \leq d$ be fixed, as an arbitrary mode for this proof. Then the tensor $T \in V_i \otimes V_{\hat{i}}$. Define a curve $$S(t):= \sum_{k=1}^{r} v^k_i(t) \otimes v_{\hat{i}}^k(t),\; S(0) = T$$Here $v_i^k(t)$ are curves in $V_i$, and $v_{\hat{i}^k}(t)$ are curves in $V_{\hat{k}}$. Now one can restrict to working with $V_i^{\prime} \subset V_i$ instead of $V_i$, and still have $S(t) \in V_i^{\prime} \otimes V_{\hat{i}}$. This implies $v_i^k(t) \in V_i^{\prime}$, making $S(t): V_i^* \to V_{\hat{i}}$ an injective mapping, when $t$ is close to $0$. Then since $T(V_i^*)$ can be written as $\lim_{t \to 0} S(t)(V_i^*) \subset \lim_{t\to 0} \langle v_{\hat{i}}^k(t) \rangle_{k=1}^r$, one has $\rank(T(V_i^*)) \leq \rank(T)$.
    
    Conversely, consider a set $w_{\hat{i}}^k(t) \subset V_{\hat{i}}$ such that $T(V_i^*) \subset \lim_{t\to 0} \langle w_{\hat{i}}^k \rangle_{k=1}^r$, where the span of $w_{\hat{i}}^k(t)$ is assumed to be constant. Then, considering a basis $\{f_{i,j}\}_{j=1}^{n_i}$ of $V_i^*$, one can write $$T = \sum_{k=1}^{r} v^k_i \otimes \left[\lim_{t\to 0}\sum_{s=1}^{n_i}\lambda_{k,s}(t)w_{\hat{i}}^k(t)\right].$$ Here the $\lambda_{k,s}(t)$ are appropriately chosen functions. By the multilinear nature of tensor products, this can be rewritten as $$S(t) = \sum_{k=1}^{r} \left[\sum_{s=1}^{n_i}\lambda_{k,s}v_i^k(t)\right] \otimes w_{\hat{i}}^k(t).$$At $t \to 0$, this forms a decomposition of $T$ which must necessarily be longer than the rank decomposition of $T$. Thus $\rank(T) \leq \rank(T(V_i^*))$.
\end{proof}
...
\begin{definition}[$i$-th slice space of $T$]\label{defn:slicespace}
    ...
    $$\mathcal{L}_i := \langle T^i_j \rangle_{j=1}^{n_1} \subseteq V_{\hat{i}},$$ or the linear span generated by all the $i$-slices of $T$.
\end{definition}
