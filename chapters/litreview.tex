...
\nomenclature[D]{$T^{(i)}$}{The flattening of a tensor $T$ along the $i$th mode}
...
The current overview of cases where Comon's Conjecture is known to hold is as follows: \begin{itemize}
	...
	\item In the case $d \geq 3,  \text{char}(\mathbb{F})  \geq 3, T \in S^d(\mathbb{F}^n)$ and $\rank(T) = \rank(T^{(i)})$ or $\rank(T^{(i)}) + 1$ (Here $T^{(i)}$ denotes the flattening \cref{subsection:tensorflattening} of $T$ along any mode, since $T$ is symmetric all directions are equivalent) \cite{friedland_remarks_2016}
