...
\end{proof}
\begin{remark}
    In the literature surveyed, flattening can be done not just for one mode, but for a collection of modes. This gives rise to the empty set notation denoting the unravelling to a vector. Since this is less important for the following constructions, this definition of flattening is not covered.
\end{remark}
...
\begin{definition}[Adjoining Slices to a Tensor along a specific mode]\label{defn:adjoiningslicesinglemode}
    ...
    Since $\mathcal{M}$ is a finite collection of slices, it can be rewritten as a tensor of order $d$, lying in $V_1 \otimes \cdots\otimes V_{i-1} \otimes \field^n \otimes V_{i+1} \otimes\cdots \otimes V_d$ in the following manner where $M \in V_1 \otimes \cdots \otimes V_d$ is defined in terms of each of the slices $M_i$
    \begin{equation}
        M(k_1, \ldots k_{i-1}, k_i, k_{i+1} \ldots , k_d) = M_{k_i}(k_1, \ldots k_{i-1}, k_{i+1} \ldots k_d)
    \end{equation}

    Define $\tau := Adj_i(T, \mathcal{M}) \in V_1 \otimes \cdots V_{i-1} \otimes \left( V_i \oplus \field^n \right) \otimes V_{i+1} \cdots \otimes V_d$ in the following manner 
    $$\tau(k_1, \ldots, k_d) = \begin{cases}
        T(k_1, \ldots, k_d) & (k_i)_{i=1}^d \in \prod_{i=1}^{d} \{1, \ldots, n_i\}\\
        M(k_1, \ldots, k_d) & (k_j)_{j \neq i} \in \prod_{j\neq i} \{1, \ldots, n_j\} \text{ and } n_i \leq k_i \leq n_i + N
    \end{cases}$$
\end{definition}
\begin{remark}
    The above definition allows for slices to be adjoined to a tensor when all of the slices can be applied along a specific mode. The next major step to be taken is adjoining slices along multiple directions. Doing so requires the definition of a "padding" operation, that allows for lower order tensors to be adjusted into appropriate format, before being adjoined in the appropriate direction.
\end{remark}
\begin{definition}
        
\end{definition}
\begin{observation}
    According to the above definition $\tau$ can be seen as an order $d$ tensor still, but now lying in $\left( V_1 \oplus \langle e_{n_1 +1}\rangle \right) \otimes \cdots \otimes V_d$ where $e_{n_1 + 1}$ indicates a "new" basis vector added onto the original vector space $V_1$.
\end{observation}
Note here that even though the definition is defined in terms of elementary tensors, the operation can be extended to tensors of any rank. \Cref{figure:adjoiningslices} illustrates the above definition.
\begin{proposition}
    Given $V := \field^{n_1} \otimes \cdots \otimes \field^{n_d}$
    The mapping $Adj: V \times \times_{i=1}^d V_{\hat{i}} \to $
\end{proposition}
\begin{observation}
    $Adj(T, \mathcal{M}_1, \ldots \mathcal{M}_d)$ can be defined analogously, where now the $\mathcal{M}_i$ indicate a family of order $d-1$ tensors of appropriate format. The \textit{symmetric adjoin} can be denoted $SAdj(T, \mathcal{M})$ where $\mathcal{M}$ is adjoined along every mode
\end{observation}
% What does this operation look like in geometry?
What are the properties of this function? (injective/surjective/domain, codomain, range)?
How does this operation change the rank of the tensor? Does it change the rank?
Does it affect any other properties apart from rank and format?
Does the order/permutation of the slices/slicing families matter?
\begin{figure}
    \centering
    \begin{tikzpicture}[scale=1]
        % Coordinates
...
Given that a tensor admits many non-unique decompositions, one interesting way to analyse the structure of a tensor is in analogue to Gaussian elimination for matrices. The substitution method as described in \cite{landsberg_abelian_2017,alexeev_tensor_2011} has been a computational tool since the 1960'. Rather than arrive at one possible "lowest" optimal point, generating a family of tensors modulo a set of slices becomes equivalent to looking at all the possible ways a tensor could be transformed, based on transforming its constituent slices.
\begin{definition}[Tensor Family generated by Slices]\label{defn:familymoduloslices}
...
\end{definition}
\begin{observation}
    Here, the family $F \subseteq V_1 \otimes \cdots \otimes V_d$, i.e. all its elements remain of the same order as the original tensor $T$. \Cref{figure:familymoduloslices} illustrates how one one such element from such a family can be generated. The number of slices (and their orientations) used to generate such a family can vary, but the definition can be simply extended.
\end{observation}
What can one say about rank and symmetric rank of the elements of this family? Maximum or minima?
...
\begin{definition}[Cloning a Tensor]\label{defn:tensorcloning}
    Let $T \in V_1 \otimes \cdots \otimes V_d$ be an order $d$-tensor, with $T = v_1 \otimes \cdots \otimes v_d$

    Define the $n$ clone of $T:= C_n(T)$ as $$C_n(T)(k_{1, j_1} | \ldots | k_{d, j_d}) = T(k_1 | \ldots | k_d)$$where $1 \leq j_i \leq n$
\end{definition}
\begin{observation}
    ...
    In fact, any tensor can be cloned, leading to the following description of cloning a standard ordered basis vector: $C_n(e_i) = \sum_{j=1}^{n} e_{i,j}$ as a mapping $V \to \bigoplus_{i=1}^n V$. \\
    $C_n(A + B) = C_n(A) + C_n(B)$ 
\end{observation} 
